...
<div class="table-responsive">
	<table class="table table-bordered table-hover">
		...
		<?php if(is_array($user)): foreach($user as $key=>$data): ?><tr><td><?php echo ($key+1); ?></td><td><?php echo ($data['username']); ?></td><td><?php echo ($data['companyname']); ?></td><td><?php echo ($data['usersex']); ?></td><td><?php echo ($data['usertel']); ?></td><td><?php echo ($data['userstation']); ?></td><td><?php echo ($data['userjob']); ?></td><td><?php echo ($data['userroom']); ?></td><td><?php echo ($data['classname']); ?><span data-id="<?php echo ($data['id']); ?>" data-classId="<?php echo ($data['classid']); ?>" onclick="getClass(this)" title="设置分组" class="badge primary"><span class="glyphicon glyphicon-edit"></span></span></td><td><!--<span class="badge primary">修改</span>--><?php if($data['status'] == 1): ?><span data-id = "<?php echo ($data['id']); ?>" onclick="unsetuser(this)" title="删除" class="badge danger">删除</span><?php else: ?><span data-id = "<?php echo ($data['id']); ?>" onclick="recoveruser(this)" title="恢复" class="badge warning">恢复</span><?php endif; ?></td></tr><?php endforeach; endif; ?>
	</table>
</div>
<script type="text/javascript">
    //默认选择
    $('#userstatus').find("option[value='<?php echo ($_GET['userstatus']); ?>']").attr('selected',true);
