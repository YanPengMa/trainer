...
class FormBuilder {
	...
	function add_input( $label, $args = '', $slug = '' ) {
		...
		$defaults = array(
			...
			'options'          => array(),
			'wrap_tag'         => 'div',
			'wrap_class'       => array( 'input-group' ),
			'wrap_id'          => '',
			'wrap_style'       => '',
			'before_html'      => '',
			'after_html'       => '',
			'request_populate' => true,
			'disabled'         =>false,//自定义添加，是否禁用输入
			'readonly'         => false,//自定义添加，禁用修改
			'data-rule' => ''//自定义添加，用于前端输入验证
		);
	...
	function build_form( $echo = false ) {
		...
		foreach ( $this->inputs as $val ) :
			...
			//自定义添加附加字段，判断value、placeholder和data-rule是否存在
			$attach = '';
			$attach .= !empty($val['value']) ? ' value="'. $val['value'] .'"' : '';
			$attach .= !empty($val['data-rule']) ? ' data-rule="'. $val["data-rule"] .'"' : '';
			$attach .= !empty($val['placeholder']) ? ' placeholder="'. $val['placeholder'] .'"' : '';
			$attach .= $val['readonly'] ? ' readonly="readonly"' : '';
					...
					if ( count( $val['options'] ) > 0 ) :
						...
						foreach ( $val['options'] as $key => $opt ) {
							...
							$end .= sprintf(
								...
								$val['type'],
								$val['name'],
								$opt,//自定义修改，将$key修改成了$opt
								$key//自定义修改，将$slug修改成了$key
							);
							//自定义添加附加字段，判断data-rule是否存在
							$end_rule = (++$i==1) ? $attach : '';
							$end .= $end_rule;
							if (
								// Is this field set to automatically populate?
								...
								$end .= ' checked';
								// Does the field have a default selected value
								//自定义添加，给radio，checkbox增加默认选中功能
							} else if ( $val['checked'] === $opt ) {
								$end .= ' checked';
							}
							$end .= $this->field_close();
							$end .= ' <label for="' . $key . '">' . $opt . '</label>';//自定义修改，将$slug修改成了$key
							$end .= '<br>';//自定义添加，radio和checkbox的每个选项后面加上<br>
						}
						//自定义增加循环
						$dataRuleArray = explode(',', $val['data-rule']);
						foreach($dataRuleArray as $value){
							//自定义修改，将判断条件$val['required']修改成data-rule内是否存在required或者checked
							if ( strstr($value, 'required') || strstr($value, 'checked') ) { 
								$val['label'] .= ' <strong class="text-danger">*</strong>';
								break;
							}
						}
						//$label_html = '<div class="checkbox_header">' . $val['label'] . '</div>';
						$label_html = '<label>' . $val['label'] . '</label><br>';//自定义修改，将radio和checkbox的标记设置为label
						break;
					endif;
			...
			if ( $val['type'] === 'submit' ) {
				...
				//自定义添加，自定义的submit需要默认使按钮居中
				$val['before_html'] = '<div class="text-center">';
				$val['after_html'] = '</div>';
			}
...
//			$attr .= $val['autofocus'] ? ' autofocus' : '';//自定义注释
			$attr .= $val['checked'] ? ' checked' : '';
//			$attr .= $val['required'] ? ' required' : '';//自定义注释
			$attr .= $val['disabled'] ? ' disabled' : '';
			...
			if ( ! empty( $element ) ) {
				if ( $val['type'] === 'checkbox' ) {
					...
					$field .= '
					<' . $element . $id . ' name="' . $val['name'] . '"' . $min_max_range . $class . $attach . $attr . $end;
				}
			// Not a form element
			} else {
				$field .= $end;
		...
		if ( ! $this->has_submit && $this->form['add_submit'] ) {
			$output .= '<div'. $this->_output_classes( $val['wrap_class'] ) .'><input type="submit" value="Submit" name="submit"></div>';
		}
